...
def getMixEmoji(text, fn):
    emoji_code_1 = hex(ord(text[0]))[2:]
    if(len(text) >= 2):
        emoji_code_2 = hex(ord(text[1]))[2:]
        if(emoji_code_2 == "fe0f" and len(text) >= 3):
            emoji_code_2 = hex(ord(text[2]))[2:]
        if(emoji_code_2 == "fe0f" and len(text) == 2):
            emoji_code_2 = "1f422"
    else:
        emoji_code_2 = "1f422"
    ...
    if(emoji_code_1 + "-fe0f" in info):
        emoji_code_1 = emoji_code_1 + "-fe0f"
    if(emoji_code_2 + "-fe0f" in info):
        emoji_code_2 = emoji_code_2 + "-fe0f"
...
async def handle_first_receive(matcher: Matcher, event: Event, msg: Message = CommandArg()):
    ...
    if(len(args) == 1):
        ...
        e = getMixEmoji(args[0], str(mix_out))
        await mix.finish(MessageSegment.image(mix_out.resolve().as_uri()))
    if(len(args) >= 2):
        power_factor = 1
        if(len(args) > 2):
            power_factor = float(args[2])
        stats = getMixEmoji(args[0], str(MIX_DIR / "img1.png"))
        if(stats < 0):
            await mix.finish("指令中可能有不存在的表情组合")
        stats = getMixEmoji(args[1], str(MIX_DIR / "img2.png"))
        if(stats < 0):
            await mix.finish("指令中可能有不存在的表情组合")
        ...
        gif_frames[0].save(str(gif_out),
                save_all=True, append_images=gif_frames[1:], optimize=False, duration=20, loop=0, transparency=0)
